...
#ifdef dynamic
    void* handle = dlopen("./liblabonelib.so", RTLD_LAZY);
    if(!handle){/*error*/}

    void (*create_table)(int size) = dlsym(handle, "create_table");
    void (*wc_files)(int num, char** filenames) = dlsym(handle, "wc_files");
    int (*to_mem)(int num) = dlsym(handle, "to_mem");
    void (*remove_block)(int id) = dlsym(handle, "remove_block");
    void (*read_table)() = dlsym(handle, "read");
    void (*remove_row)(int idb, int idr) = dlsym(handle,"remove_row");
    void (*free_table)() = dlsym(handle,"remove_row");
