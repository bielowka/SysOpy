...
int main(int argc, char *argv[]){
    ...
    if (strcmp(argv[2], "unix") == 0){
        ...
        int a = getaddrinfo("127.0.0.1", port, &hints, &info);
        if (a != 0) {
            printf("(%s)\n", gai_strerror(a));
            exit(0);
        }
    ...
    if (strcmp(argv[2], "unix") == 0)
    {
        sendto(binded_socket, msg, MSG_LEN, 0, (struct sockaddr *)&local_sockaddr, sizeof(struct sockaddr_un));
    }
    else
    {
    ...
    while(true){
        if (strcmp(argv[2], "unix") == 0)
        {
            recv(binded_socket, buffer, MSG_LEN, 0);
        }
        else
        {
